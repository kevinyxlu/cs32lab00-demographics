...
void readCSVLineDemog(std::string theLine, vector<string>& theCounties, vector<double>& thePopOver65, vector<double>& thePopUnder18) {
    ...
    thePopUnder18.push_back(popUnder18);
}


//read from a CSV file into parallel vectors
void read_csv(std::string filename, vector<string>& theCounties, vector<double>& thePopOver65, vector<double>& thePopUnder18) {
    ...
    if(myFile.good()) {
        ...
        while(std::getline(myFile, line)) { //keeps looping until there are no more lines to get
            readCSVLineDemog(line, theCounties, thePopOver65, thePopUnder18);
